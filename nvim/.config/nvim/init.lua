...
local learn_md_path = {
  vim.fn.expand '~/notes/learning/mistakes.md',
  vim.fn.expand '~/tests/pwn.md',
}

vim.api.nvim_create_autocmd('BufWinEnter', {
  pattern = learn_md_path,
  callback = function()
...
vim.api.nvim_set_keymap('n', '<leader>P', '"+P', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', '<leader>m', '"*p', { noremap = true, silent = true })

vim.api.nvim_create_user_command('Summarycopy', function()
  vim.fn.setreg('A', '')
  vim.cmd [[g/#/y A]]
end, {})
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      require('telescope').setup {
        ...
        pickers = {
          find_files = {
            ...
            no_ignore = true, -- do not ignore .gitignored files
      ...
      end, { desc = 'Grep in current buffer dir' })

      vim.keymap.set('n', '<leader>sc', function()
        builtin.find_files {
          prompt_title = 'Search .config',
          cwd = vim.fn.expand '~/.config', -- or os.getenv("HOME") .. "/.config""HOME") .. "/.config"
        }
      end, { desc = '[S]earch in entire projects folder' })
      vim.keymap.set('n', '<leader>sp', function()
        builtin.find_files {
          prompt_title = '[S]earch in projects',
          cwd = vim.fn.expand '~/projects', -- or os.getenv("HOME") .. "/.config"
        }
      end, { desc = '[S]earch projects folder' })
      ...
      -- ──→ With this new multi-dir <leader>sn mapping:
      vim.keymap.set('n', '<leader>sf', function()
        require('telescope.builtin').find_files {
          prompt_title = '[S]earch Notes & Journal',
          find_command = {
            'fd',
            '--type',
            'f',
            ...
            -- add more dirs here if you like
          },
        }
      ...
      local servers = {
        clangd = {
          settings = {
            clangd = {
              completion = {
                -- Adjust completion style to potentially reduce extra keywords
                style = 'minimal', -- Try "minimal" or "detailed" to see if it affects restrict visibility
              },
            },
          },
        },
    ...
    config = function()
      ...
      require('tokyonight').setup {
        ...
        on_colors = function(colors)
          colors.bg = '#0a0a14' -- darker than default
          colors.bg_dark = '#0a0a14'
        end,
      }
